-- ============================================
-- Village Voice Community Seed Data - SIMPLE VERSION
-- This script automatically finds topic IDs
-- ============================================
-- 
-- INSTRUCTIONS:
-- 1. Get a user ID first: SELECT id FROM auth.users LIMIT 1;
-- 2. Replace 'YOUR_USER_ID_HERE' below with that UUID
-- 3. Run this entire script
--
-- ============================================

-- First, let's get the topic IDs and store them
DO $$
DECLARE
  topic_pregnancy_id UUID;
  topic_ttc_id UUID;
  topic_baby_id UUID;
  topic_birth_month_id UUID;
  seed_user_id UUID := 'YOUR_USER_ID_HERE'; -- REPLACE THIS with your user ID
BEGIN
  -- Get topic IDs
  SELECT id INTO topic_pregnancy_id FROM topics WHERE slug = 'pregnancy-journey' LIMIT 1;
  SELECT id INTO topic_ttc_id FROM topics WHERE slug = 'trying-to-conceive' LIMIT 1;
  SELECT id INTO topic_baby_id FROM topics WHERE slug = 'baby-care' LIMIT 1;
  SELECT id INTO topic_birth_month_id FROM topics WHERE slug = 'birth-month-clubs' LIMIT 1;

  -- Check if we have all topics
  IF topic_pregnancy_id IS NULL OR topic_ttc_id IS NULL OR topic_baby_id IS NULL OR topic_birth_month_id IS NULL THEN
    RAISE EXCEPTION 'One or more topics not found. Please check your topics table.';
  END IF;

  -- Check if user ID is set
  IF seed_user_id = 'YOUR_USER_ID_HERE' THEN
    RAISE EXCEPTION 'Please replace YOUR_USER_ID_HERE with an actual user ID from auth.users';
  END IF;
